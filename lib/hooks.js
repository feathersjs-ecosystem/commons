...
function makeConverter (keys) {
  return args => args.reduce((result, value, index) => ({ ...result, [keys[index]]: value }), { params: {} });
}

// Converters from service method arguments to hook object properties
exports.converters = {
  find: makeConverter(['params']),
  create: makeConverter(['data', 'params']),
  get: makeConverter(['id', 'params']),
  remove: makeConverter(['id', 'params']),
  update: makeConverter(['id', 'data', 'params']),
  patch: makeConverter(['id', 'data', 'params'])
};
...
exports.createHookObject = function createHookObject (method, args, data = {}) {
  const serviceConverters = data.service && data.service.converters;
  const converters = Object.assign({}, exports.converters, serviceConverters);
